          ...
          {filteredProducts.map((product) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                {/* Buying Price and Profit Information */}
                <div className="space-y-1 mb-2">
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-500 dark:text-gray-400">Cost:</span>
                    <span className="text-gray-600 dark:text-gray-300">{formatCurrency(product.buyingPrice)}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-green-600 dark:text-green-400 font-medium">Profit:</span>
                    <span className="text-green-600 dark:text-green-400 font-medium">
                      {formatCurrency(product.price - (product.buyingPrice ?? 0))}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-blue-600 dark:text-blue-400">Margin:</span>
                    <span className="text-blue-600 dark:text-blue-400">
                      {product.buyingPrice ? ((product.price - product.buyingPrice) / product.price * 100).toFixed(1) : 'N/A'}%
                    </span>
                  </div>
                </div>
