...
const ExportModal: React.FC<ExportModalProps> = ({ data, period, onClose }) => {
  ...
  const exportToPDF = async () => {
    try {
      ...
      doc.text(`Total Profit: ${formatCurrency((data.totalProfit || 0))}`, 20, 120);
      doc.text(`Expenses: ${formatCurrency((data.totalExpenses || 0))}`, 20, 130);
      doc.text(`Net Profit: ${formatCurrency((data.netProfit || 0))}`, 20, 140);

      // Top Products
      doc.setFontSize(16);
      doc.text('Top Products', 20, 160);
