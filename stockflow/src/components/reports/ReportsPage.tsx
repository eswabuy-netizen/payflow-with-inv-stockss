...
const ReportsPage: React.FC = () => {
  ...
  const loadAnalytics = async () => {
    ...
    setLoading(true);
    try {
      const analyticsData = await salesService.getSalesAnalytics(period, currentUser.companyId);
      // Compute matching expense window
      const now = new Date();
      let startDate: Date;
      switch (period) {
        case 'daily':
      ...
      const totalExpenses = await expensesService.getExpenseTotals(currentUser.companyId, startDate, now);
      const netProfit = (analyticsData.totalProfit || 0) - (totalExpenses || 0);
      (analyticsData as any).totalExpenses = totalExpenses;
      (analyticsData as any).netProfit = netProfit;
      
      // Debug logging
      console.log('Analytics data:', analyticsData);
      console.log('Total profit:', analyticsData.totalProfit);
      console.log('Average profit per transaction:', analyticsData.averageProfitPerTransaction);
      console.log('Top products:', analyticsData.topProducts);
      console.log('Total expenses:', (analyticsData as any).totalExpenses);
      console.log('Net profit:', (analyticsData as any).netProfit);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-xl lg:text-2xl font-bold text-gray-900 dark:text-white">
                  {formatCurrency(isNaN(analytics.totalProfit) ? 0 : analytics.totalProfit)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-xl lg:text-2xl font-bold text-gray-900 dark:text-white">
                  {formatCurrency((analytics as any).totalExpenses || 0)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-xl lg:text-2xl font-bold text-gray-900 dark:text-white">
                  {formatCurrency((analytics as any).netProfit || 0)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-xl lg:text-2xl font-bold text-gray-900 dark:text-white">
                  {formatCurrency(isNaN(analytics.averageProfitPerTransaction) ? 0 : analytics.averageProfitPerTransaction)}
                </p>
              </div>
            </div>
          </div>
                ...
                {analytics.topProducts.map((product, index) => (
                  <tr key={product.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-600 dark:text-green-400">
                      {formatCurrency(isNaN(product.profit) ? 0 : product.profit)}
                    </td>
                  </tr>
                ))}
              </tbody>
