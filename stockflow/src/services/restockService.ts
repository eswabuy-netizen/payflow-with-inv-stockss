...
export const restockService = {
  ...
  // Get restock history for a company
  async getRestockHistory(companyId: string): Promise<RestockEntry[]> {
    const q = query(
  ...
  // Get restock history for specific product in a company
  async getProductRestockHistory(productId: string, companyId: string): Promise<RestockEntry[]> {
    const restocks = await this.getRestockHistory(companyId);
