    ...
    if (signatureUrl) {
      ...
      img.onload = () => {
        try {
          ...
          pdf.addImage(img, 'PNG', x, y - 18, 60, 20);
          
          // Add signature text below with proper spacing
          pdf.setTextColor(color);
          pdf.setFontSize(8);
          pdf.setFont('helvetica', 'normal');
          pdf.text(companyName, x, y + 8);
          pdf.text(new Date().toLocaleDateString(), x, y + 16);
        } catch (error) {
...
const addTextSignature = (pdf: jsPDF, x: number, y: number, companyName: string, color: string) => {
  ...
  // Signature text with proper spacing
  pdf.setTextColor(color);
  pdf.setFontSize(10);
  pdf.text('Authorized Signature', x, y - 8);
  pdf.setFontSize(8);
  pdf.text(companyName, x, y + 8);
  pdf.text(new Date().toLocaleDateString(), x, y + 16);
    ...
    if (stampUrl) {
      ...
      img.onload = () => {
        try {
          ...
          pdf.addImage(img, 'PNG', x, y - 12, 30, 30);
          
          // Add stamp text below with proper spacing
          pdf.setTextColor(color);
          pdf.setFontSize(8);
          pdf.setFont('helvetica', 'normal');
          pdf.text(text, x + 15, y + 25, { align: 'center' });
        } catch (error) {
...
const addTextStamp = (pdf: jsPDF, x: number, y: number, text: string, color: string) => {
  ...
  // Create a circular stamp effect with better positioning
  pdf.setDrawColor(color);
  ...
  pdf.setTextColor(color);
  pdf.setFontSize(7);
  pdf.text(new Date().toLocaleDateString(), x + 15, y + 30, { align: 'center' });
...
export const generateInvoicePDF = async (
  ...
  // Footer background with increased height for better spacing
  pdf.setFillColor(primaryColor);
  pdf.rect(0, footerY - 5, 210, 35, 'F');
  
  // Digital signature and stamp with better positioning
  await addDigitalSignature(pdf, 15, footerY + 8, company.name, '#ffffff', company.digitalSignature);
  await addDigitalStamp(pdf, 150, footerY + 8, 'OFFICIAL INVOICE', '#ffffff', company.digitalStamp);
  
  // Footer text with proper spacing - left side
  pdf.setTextColor('#ffffff');
  pdf.setFontSize(8);
  pdf.setFont('helvetica', 'normal');
  pdf.text('Thank you for your business!', 15, footerY + 20);
  
  // Footer text - right side
  pdf.text(`Generated on ${new Date().toLocaleDateString()}`, 195, footerY + 20, { align: 'right' });
...
export const generateReceiptPDF = async (
  ...
  await addDigitalSignature(pdf, 15, footerY + 8, company.name, '#ffffff', company.digitalSignature);
  await addDigitalStamp(pdf, 150, footerY + 8, 'PAYMENT RECEIVED', '#ffffff', company.digitalStamp);
  
  pdf.setTextColor('#ffffff');
  pdf.setFontSize(8);
  pdf.text('Thank you for your payment!', 15, footerY + 20);
  pdf.text(`Generated on ${new Date().toLocaleDateString()}`, 195, footerY + 20, { align: 'right' });
...
export const generateQuotationPDF = async (
  ...
  await addDigitalSignature(pdf, 15, footerY + 8, company.name, '#ffffff', company.digitalSignature);
  await addDigitalStamp(pdf, 150, footerY + 8, 'QUOTATION', '#ffffff', company.digitalStamp);
  
  pdf.setTextColor('#ffffff');
  pdf.setFontSize(8);
  pdf.text('We look forward to working with you!', 15, footerY + 20);
  pdf.text(`Generated on ${new Date().toLocaleDateString()}`, 195, footerY + 20, { align: 'right' });
...
export const generateInvoicePDFAsBlob = async (
  ...
  pdf.text('Subtotal:', totalsBoxX + 5, yPosition + 8);
  pdf.text(formatCurrency(invoice.subtotal), totalsBoxX + 70, yPosition + 8, { align: 'right' });
  
  pdf.text(`VAT (${invoice.tax}%):`, totalsBoxX + 5, yPosition + 16);
  pdf.text(formatCurrency(invoice.subtotal * (invoice.tax / 100)), totalsBoxX + 70, yPosition + 16, { align: 'right' });
  
  pdf.setFontSize(12);
  pdf.setFont('helvetica', 'bold');
  pdf.setTextColor(primaryColor);
  pdf.text('TOTAL:', totalsBoxX + 5, yPosition + 26);
  pdf.text(formatCurrency(invoice.total), totalsBoxX + 70, yPosition + 26, { align: 'right' });
  ...
  await addDigitalSignature(pdf, 15, footerY + 8, company.name, '#ffffff', company.digitalSignature);
  await addDigitalStamp(pdf, 150, footerY + 8, 'INVOICE', '#ffffff', company.digitalStamp);
